...
from megatron.core.transformer.torch_norm import NoNorm, ScaleOnlyNorm
...
class TENorm:
    ...
    def __new__(cls, config: TransformerConfig, hidden_size: int, eps: float = 1e-5):
        ...
        if config.normalization == "NoNorm":
            instance = NoNorm()
            ...
            raise Exception("Only LayerNorm, RMSNorm, ScaleOnly and NoNorm are curently supported")
...
class TELayerNormColumnParallelLinear(te.pytorch.LayerNormLinear):
    ...
    def __new__(cls, *args, **kwargs):
        ...
        normalization = getattr(config, "normalization", None) if config is not None else None
        if normalization == "NoNorm":
            return TEColumnParallelLinear(*args, **kwargs)
        if normalization == "ScaleOnly":
            return TEScaleColumnParallelLinear(*args, **kwargs)
...
class TEScaleColumnParallelLinear(torch.nn.Module):
    """Scale-only + TE column-parallel linear fallback for non-TE norms."""
    ...
    def __init__(
        ...
        *,
        ...
        self.layer_norm_weight = Parameter(torch.ones(input_size, dtype=config.params_dtype))
        if config.sequence_parallel:
            setattr(self.layer_norm_weight, "sequence_parallel", config.sequence_parallel)
    ...
    def forward(self, x):
        gamma = self.layer_norm_weight + 1.0 if self.zero_centered_gamma else self.layer_norm_weight
        return self.linear(x * gamma)
