...
from verl.custom_optimizer.muon import assert_fsdp_orig_params_effective_for_muon
from verl.custom_optimizer.pion import assert_fsdp_orig_params_effective_for_pion
from verl.workers.config import FSDPCriticConfig, FSDPEngineConfig, HFModelConfig, RolloutConfig
from verl.workers.config.optimizer import build_optimizer
from verl.workers.engine.fsdp.utils import coerce_fsdp_use_orig_params
...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def __init__(self, config: DictConfig, role: str, **kwargs):
        ...
        print(f"actor use_orig_params: {self.use_orig_params}")
    ...
    def _build_model_optimizer(
        ...
        if fsdp_strategy == "fsdp":
            ...
            assert_fsdp_orig_params_effective_for_pion(
                actor_module_fsdp,
                intended_use_orig=use_orig_for_fsdp,
                role=role,
        ...
        if role == "actor" and optim_config is not None:
            from verl.custom_optimizer.muon import prepare_muon_module_tags
            from verl.custom_optimizer.pion import prepare_pion_module_tags
            from verl.utils.torch_functional import get_constant_schedule_with_warmup, get_cosine_schedule_with_warmup

            prepare_pion_module_tags(
                actor_module_fsdp,
                optimizer_name=str(optim_config.get("optimizer", "") or ""),
                override_optimizer_config=optim_config.get("override_optimizer_config"),
            )
