...
def _validate_sharding_for_key(rank_sharding: List[Tuple[int, ShardedTensor]]):
    ...
    # shard_access_cnt = _compute_shards_access(rank_sharding)
    # if torch.all(shard_access_cnt == 1):
    #     return  # normal: each chunk owned by exactly one rank
    # # Replicated single-chunk: all contributors hold the same full tensor (e.g. Pion exp_avg_in/exp_avg_out).
    # # Use num_contributors so we allow when this key has multiple (rank, sharding) per rank (e.g. ChainedOptimizer).
    # num_contributors = len([r for r, s in rank_sharding if is_main_replica(s.replica_id)])
    # if shard_access_cnt.numel() == 1 and shard_access_cnt.item() == num_contributors:
    #     return
    # raise CheckpointingException(
    #     f"Invalid access pattern for {rank_sharding[0][1]}: {shard_access_cnt}"
    # )
