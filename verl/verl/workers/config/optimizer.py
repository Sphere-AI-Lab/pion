...
class FSDPOptimizerConfig(OptimizerConfig):
    ...
    Args:
        ...
        eps (float): Adam-style epsilon; passed to hybrid optimizers such as PionOptimizer (AdamW branch).
    """
...
def build_optimizer(parameters, config: FSDPOptimizerConfig):
    ...
    Examples:
        ...
        # Pion (matrix rotation + AdamW); betas/eps follow top-level optim.* unless overridden below
        config.optimizer_impl = "verl.custom_optimizer.pion"
        config.optimizer = "PionOptimizer"
    """
    import importlib
    ...
    # PionOptimizer is not named like Adam* but uses betas on both matrix and vector groups.
    if (
        "adam" in optimizer_name_lower
        or "ademamix" in optimizer_name_lower
        or "pion" in optimizer_name_lower
        or "muon" in optimizer_name_lower
    ):
        optimizer_args["betas"] = config.betas
    if "pion" in optimizer_name_lower or "muon" in optimizer_name_lower:
        if OmegaConf.is_config(config):
            optimizer_args["eps"] = float(OmegaConf.select(config, "eps", default=1e-8))
        else:
            optimizer_args["eps"] = float(getattr(config, "eps", 1e-8))
    ...
    try:
        ...
        # print(f"build_optimizer: optimizer_cls={optimizer_cls}")
    ...
    if "pion" in optimizer_name_lower or "muon" in optimizer_name_lower:
        try:
            ...
            if any(isinstance(p, FlatParameter) for p in params_list):
                raise RuntimeError(
                    "Pion / PionAmbient optimizer was given FSDP FlatParameter tensors (typically 1D). "
                    "Use FSDP1 (actor.strategy=fsdp) with use_orig_params=True on the same module "
                    "that is passed to build_optimizer (see assert_fsdp_orig_params_effective_for_pion)."
                )
        except ImportError:
            pass

    return optimizer_cls(params_list, **optimizer_args)
