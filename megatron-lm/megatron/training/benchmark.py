...
class BenchmarkTask(ABC):
    ...
    def _resolve_data_path(self) -> str:
        benchmark_root = getattr(self.args, "benchmark_root", None)
...
class BenchmarkRunner:
    ...
    def __init__(self, model, args):
        ...
        # 改为（例如默认 2048，可按需要改）：
        _seq = getattr(args, "benchmark_sequence_length", None) or getattr(args, "seq_length", None)
        if _seq is None:
            _seq = 2048  # 或 1024，与训练一致即可
        self.seq_length = int(_seq)
