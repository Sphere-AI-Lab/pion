...
class TorchCommonSaveStrategy(SaveCommonStrategy):
    ...
    def save_common(self, common_state_dict: StateDict, checkpoint_dir: Union[str, Path]):
        ...
        if torch.distributed.get_rank() == 0:
            ...
            # Bug fix: 在保存之前清理不可序列化的对象（如 DataLoader 迭代器）
            # 这可以解决 eval 后 save checkpoint 时的序列化错误
            cleaned_state_dict = self._clean_common_state_dict(common_state_dict)
            
            if MultiStorageClientFeature.is_enabled():
                msc = MultiStorageClientFeature.import_package()
                msc.torch.save(cleaned_state_dict, path)
            else:
                torch.save(cleaned_state_dict, path)
    
    def _clean_common_state_dict(self, common_state_dict: StateDict) -> StateDict:
        """清理 common_state_dict 中不可序列化的对象（如 DataLoader 迭代器）"""
        import copy
        
        cleaned = {}
        for key, value in common_state_dict.items():
            if key == 'args':
                # args 需要特殊处理：移除不可序列化的对象
                if hasattr(value, '__dict__'):
                    ...
                    for k, v in vars(value).items():
                        ...
                        if 'iterator' in k.lower() or 'dataloader' in k.lower():
                            continue
                        args_dict[k] = v
                    cleaned[key] = args_dict
                else:
                    cleaned[key] = value
            elif key == 'rerun_state_machine':
                # rerun_state_machine 需要特殊处理：移除不可序列化的对象
                if value is not None:
                    if isinstance(value, dict):
                        cleaned_rerun = {}
                        for k, v in value.items():
                            # 特别处理 data_iterator_checkpoints，可能包含不可序列化的对象
                            if k == 'data_iterator_checkpoints' and v is not None:
                                # data_iterator_checkpoints 应该已经是可序列化的列表
                                # 但如果包含不可序列化的对象，跳过
                                try:
                                    copy.deepcopy(v)
                                    cleaned_rerun[k] = v
                                ...
                                try:
                                    ...
                                    # 如果无法深拷贝，跳过这个键
                                    continue
                        cleaned[key] = cleaned_rerun
                    else:
                        # 如果不是字典（可能是 ShardedObject），尝试直接复制
                        try:
                            copy.deepcopy(value)
                            cleaned[key] = value
                        except (TypeError, NotImplementedError):
                ...
                try:
                    ...
                    # 如果无法深拷贝，记录警告但继续
                    logger.warning(f"Skipping non-serializable key '{key}' in common_state_dict")
                    continue
